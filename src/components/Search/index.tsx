...
interface SearchProps {
  handleSetResult: (data: SearchResultProps[]) => void
  handleNoDefinitionFound: (notFoundObj: NotFoundProps) => void;
}

export default function Search({ handleSetResult, handleNoDefinitionFound }: SearchProps) {
  ...
  async function handleSubmit() {
    ...
    if (input === '') {
      ...
      const dataResult = await fetchDictionary(input)
      if (dataResult.title === "No Definitions Found"){
        handleNoDefinitionFound(dataResult)
      } else {
        handleSetResult(dataResult)
      }
    }
  }
    ...
    <SearchContainer $isemptyerror={isEmpty} role="search">
      ...
      >
        ...
        <button 
          ...
          type="submit"
        >
            <svg role="img" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
      ...
      {
        isEmpty && <SpanError role="alert">Whoops, can’t be empty…</SpanError>
      }
